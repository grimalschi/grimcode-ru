...
describe('auth administrator protocol', () => {
  ...
  it('requires the literal CSRF header and this surface’s cookie to match', () => {
    const headers = new Headers({
      cookie: 'installation_csrf_auth=token-value; installation_csrf_other=other-value',
      'x-csrf-token': 'token-value',
    });
    expect(isCsrfValid(headers, 'installation_csrf_auth')).toBe(true);
    expect(isCsrfValid(headers, 'installation_csrf_other')).toBe(false);
    headers.set('x-csrf-token', 'short');
    expect(isCsrfValid(headers, 'installation_csrf_auth')).toBe(false);
