...
/**
 * Every admin denial is a 403 — Router never leaks whether the refusal came from a missing
 * session, a missing grant or an owner-only module.
 */
export function forbidden(request: Request, publicOrigin: string): Response {
  const signIn = `${publicOrigin}/app/login`;
  return respond(
    request,
    403,
    'forbidden',
...
export function moduleUnavailable(request: Request): Response {
  return respond(
    request,
    503,
    'module-unavailable',
...
export function badGateway(request: Request): Response {
  return respond(request, 502, 'bad-gateway', 'Module unreachable', 'The module did not answer.');
}
