...
export function LoginScreen() {
  ...
  return (
    <AuthCard
      ...
      footer={
        <>
          <Link to="/app/register" className="text-sm underline underline-offset-4">
            Создать аккаунт
          </Link>
          <Link to="/app/reset-password" className="text-sm underline underline-offset-4">
            Забыли пароль?
          </Link>
        </>
      }
...
export function RegisterScreen() {
  ...
  return (
    <AuthCard
      ...
      footer={
        <Link to="/app/login" search={{ next: undefined }} className="text-sm underline underline-offset-4">
...
export function ResetPasswordScreen() {
  const search = useSearch({ from: '/app/reset-password_/confirm' });
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      void navigate({ to: '/app/login', search: { next: undefined } });
    } catch (error) {
      toast.error(messageOf(error));
      setBusy(false);
    }
...
export function ConfirmEmailChangeScreen() {
  const search = useSearch({ from: '/app/confirm-email-change' });
  ...
  if (state === 'failed') {
    return (
      <AuthCard
        ...
        description={message}
        footer={
          <Link to="/app/settings" className="text-sm underline underline-offset-4">
...
export function VerifyEmailScreen() {
  const search = useSearch({ from: '/app/verify-email' });
  const { refresh } = useSession();
  const [state, setState] = React.useState<'working' | 'done' | 'failed'>('working');
  const [message, setMessage] = React.useState('');
