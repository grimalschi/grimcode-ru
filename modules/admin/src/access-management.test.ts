...
function fixture() {
  ...
  const repo = {
    ...
    withRegistryLock: <T>(operation: (repository: unknown) => Promise<T>) => {
      const result = pending.then(() => operation(repo));
  ...
  const auth = {
    ...
    getIdentitiesByIds: async ({ ids }: { ids: string[] }) => ({ identities: identities.filter((identity) => ids.includes(identity.id)) }),
  ...
  function caller(id: string) {
    return adminRouter.createCaller({
      repo, auth, catalogue: [], env: { sessionCookieName: 'session', csrfCookieName: 'csrf', publicOrigin: 'https://test' },
      request: new Request('https://test/admin/rpc', { headers: { cookie: `session=${id}; csrf=value`, 'x-csrf-token': 'value' } }),
      resHeaders: new Headers(), admin: { userId: id, email: `${id}@current.test`, role: 'owner' },
    } as unknown as AdminRpcContext);
  }
  return { rows, identities, caller, repo };
}
