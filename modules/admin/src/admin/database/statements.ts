...
function whole(value: unknown, fallback: number, name: string): number {
  ...
  if (typeof value !== 'number' || !Number.isInteger(value)) {
    throw new TRPCError({ code: 'BAD_REQUEST', message: `${name} is a whole number.` });
  }
  return value;
}
...
export function selectRows(
  ...
  const columns = returnedColumns(table);
...
export function insertRow(table: Table, body: { values?: unknown }): Statement {
  if (typeof body.values !== 'object' || body.values === null || Array.isArray(body.values)) {
    throw new TRPCError({ code: 'BAD_REQUEST', message: 'Values are an object of column names and values.' });
  }
  ...
  if (missing.length > 0) {
    throw new TRPCError({ code: 'BAD_REQUEST', message: `${missing.map((column) => column.name).join(', ')} ` +
  ...
  const placeholders = named
    ...
    .join(', ');
  const returned = returnedColumns(table);
...
function keyClause(table: Table, key: unknown, parameters: Parameters): string {
  ...
  if (typeof key !== 'object' || key === null || Array.isArray(key)) {
    throw new TRPCError({ code: 'BAD_REQUEST', message: 'A key is an object of column names and values.' });
  }
  ...
  if (missing.length > 0 || extra.length > 0) {
    throw new TRPCError({ code: 'BAD_REQUEST', message: `The key of ${table.schema}.${table.name} is (${table.primaryKey.join(', ')}); ` +
  ...
  return table.primaryKey
    .map((column) => {
      ...
      if (value === null || value === undefined) {
        throw new TRPCError({ code: 'BAD_REQUEST', message: `The key column ${column} has no value.` });
      }
...
export function updateRow(
  ...
  if (names.length === 0) throw new TRPCError({ code: 'BAD_REQUEST', message: 'Nothing to change.' });

  const keyColumns = names.filter((name) => table.primaryKey.includes(name));
  if (keyColumns.length > 0) {
    throw new TRPCError({ code: 'BAD_REQUEST', message: `${keyColumns.join(', ')} identifies the row and cannot be changed here.` });
  }
