...
/**
 * The catalogue of one configured schema: columns and primary keys, no joins between them. Only `BASE TABLE` —
 * a view has no key to address a row by, and this interface changes rows.
 *
 * Read on every request: schema changes can be applied while this interface is running, so a
 * catalogue cached at startup would describe a schema that no longer exists.
 */
