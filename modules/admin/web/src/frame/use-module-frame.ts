 ...
 *
 * The shell owns the theme and the module-relative route; the iframe owns everything inside it.
 * Messages are only ever accepted from this window's own origin and from this iframe's window.
 */
export interface ModuleFrameOptions {
  frame: React.RefObject<HTMLIFrameElement | null>
  /** Module-relative path the shell wants the iframe to show. */
  path: string
...
export interface ModuleFrameState {
  loading: boolean
...
export function useModuleFrame({
  ...
  theme,
  onPathChange,
}: ModuleFrameOptions): ModuleFrameState {
  ...
  React.useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      const message = event.data as ChildFrameMessage | null
  ...
  React.useEffect(() => {
    const wanted = normalizeModulePath(path)
