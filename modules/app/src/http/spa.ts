 ...
 *
 * Deep links inside the SPA fall back to `index.html`, so `/app/settings` renders the app instead
 * of a 404. The browser checks the session before showing a protected screen; each API endpoint
 * independently checks access to its data.
 */
export function mountSpa(
  ...
  app.get(`${base}/*`, async (c) => {
    ...
    try {
      ...
      return c.json(
        { error: 'not-built', message: 'The application has not been built yet.' },
        503,
      );
    }
