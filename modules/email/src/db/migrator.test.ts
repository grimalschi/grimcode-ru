...
describe('email migration storage convention', () => {
  ...
  it('locks and rechecks before applying a new version, then records it in the same transaction', async () => {
    ...
    expect(database.client.query).toHaveBeenCalledWith('SELECT pg_advisory_xact_lock(hashtext($1))', ['module-migrations:email']);
