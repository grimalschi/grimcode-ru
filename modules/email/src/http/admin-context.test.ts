...
describe('email administrator protocol', () => {
  ...
  it('requires the literal CSRF header and this surface’s cookie to match', () => {
    const headers = new Headers({
      cookie: 'installation_csrf_email=token-value; installation_csrf_other=other-value',
      'x-csrf-token': 'token-value',
    });
    expect(isCsrfValid(headers, 'installation_csrf_email')).toBe(true);
    expect(isCsrfValid(headers, 'installation_csrf_other')).toBe(false);
    headers.set('x-csrf-token', 'short');
    expect(isCsrfValid(headers, 'installation_csrf_email')).toBe(false);
